######################### NOTE THIS IS A SPECIAL ARCHITECTURE, SOMEONE SUBMITTED IT AS IMAGE_TEXT_TO_TEXT
# IT'S ACTUALLY VIDEO_TEXT_TO_TEXT THAT IS WHY IT CAN IMPORT FROM THAT TASK AND STILL WORK
# NOTE it's important to understand that our taxonomy is actually a graph
# all that really matters for things to work is knowing the model architecture and its sub task (default, or chat)
# however, for the sake of the user experience we group everything into a tree
# any to any models are an example of something that would import adapters as a graph depending on what modalities are present
